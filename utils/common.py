...
def draw_box(out, boxes, color, margin, W0, H0, right=False, matched=False):
    for box in boxes:
        ...
        cv2.rectangle(out, (min_x, min_y, box_w, box_h), color, 3)
...
def visualize(image0, image1, boxes0, boxes1, matches):
    ...
    for match in matches:
        ...
        cv2.rectangle(out, (min_x0, min_y0, box_w0, box_h0), green, 3)
        cv2.rectangle(out, (min_x1 + margin + W0, min_y1, box_w1, box_h1), green, 3)
        cv2.line(out, (x_center0, y_center0), (x_center1 + margin + W0, y_center1), color,
